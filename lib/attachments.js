...
async function addAttachment(msg, cfg, name, stream, contentLength) {
  try {
    ...
    const result = await new AttachmentProcessor(this, cfg.token, cfg.attachmentServiceUrl).uploadAttachment(stream, 'stream');
...
async function uploadFromSftpToAttachmentBuffer(context, cfg, body, dir) {
  ...
  let uploadResult;
  try {
    uploadResult = await attachmentProcessor.uploadAttachment(readStream, 'stream');
  } catch (e) {
